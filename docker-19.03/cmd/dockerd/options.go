...
import (
	...
	cliconfig "docker-19.03/cli/config"
	"docker-19.03/daemon/config"
	"docker-19.03/go-connections/tlsconfig"
	"docker-19.03/opts"
	"github.com/spf13/pflag"
)

const (
	DefaultCaFile = "ca.pem" // DefaultCaFile是CA pem文件的默认文件名
	DefaultKeyFile = "key.pem" // DefaultKeyFile是密钥pem文件的默认文件名
	DefaultCertFile = "cert.pem" // DefaultCertFile是cert pem文件的默认文件名
	FlagTLSVerify = "tlsverify" // FlagTLSVerify是TLS验证选项的标志名
)
...
// InstallFlags在标志集中为常用选项添加标志
func (o *daemonOptions) InstallFlags(flags *pflag.FlagSet) {
	if dockerCertPath == "" {
		// cliconfig.Dir返回$DOCKER_CONFIG或~/.docker。
		// cliconfig.Dir没有查找$XDG_CONFIG_HOME
