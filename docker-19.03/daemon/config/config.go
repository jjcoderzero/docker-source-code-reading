...
import (
	...
	daemondiscovery "docker-19.03/daemon/discovery"
	"docker-19.03/opts"
	"docker-19.03/pkg/authorization"
	"docker-19.03/pkg/discovery"
	"docker-19.03/registry"
...
// CommonConfig定义了跨平台通用的docker守护进程的配置。它包含json标记，用于对来自使用与命令行标记相同名称的文件的配置进行反序列化。
type CommonConfig struct {
	...
	DNSConfig
	LogConfig
	BridgeConfig // bridgeConfig保存桥接网络特定配置。.
	NetworkConfig
...
func getConflictFreeConfiguration(configFile string, flags *pflag.FlagSet) (*Config, error) {
	...
	if flags != nil {
		...
		// See https://docker-19.03/issues/20289 for an example.
		//
